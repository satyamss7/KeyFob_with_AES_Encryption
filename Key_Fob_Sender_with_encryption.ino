...
int button_state_1 = 0;   // variable for reading the button status
char transmit_string[128] = "{\"DD\":0,\"AD\":0,\"DW\":0,\"AW\":0,\"TD\":0,\"HD\":0,\"SD\":0,\"Dist\":00.00}";
...
void setup() {
  // DEBUG monitoring
  pinMode(BUTTON_1, INPUT); //configuring it as input
...
/*{\"DD\":1,\"AD\":1,\"DW\":0,\"AW\":0,\"TD\":0,\"HD\":0,\"SD\":1,\"Dist\":00.00}
zero - no button press
DD - 1 Lock Driver Door
DD - 2 Unlock Driver Door
AD - 1 Lock All Doors
AD - 2 Unlock All Doors
TD - 1 Unlock Trunk Door
HD - 1 Unlock Hood
*/
...
void transmitter() {
  ...
  DW1000.newTransmit();
  DW1000.setDefaults();
  String msg = "{\"DD\":1,\"AD\":1,\"DW\":0,\"AW\":0,\"TD\":0,\"HD\":0,\"SD\":1,\"Dist\":00.00}";// msg += sentNum;

  //char encrypted_data[128] = {'\0'};
  //memcpy(encrypted_data,tempBuff,strlen(tempBuff));
 
  //Serial.println((char)encrypted_data[0]);
  //test_encrypt_cbc();
  //test_decrypt_cbc();
  char encrypted_data[128];
  memcpy(encrypted_data,in,sizeof(in));
  //Serial.println(encrypted_data);
  DW1000.setData(encrypted_data);
...
/*void transmitter2() {
  ...
  DW1000.newTransmit();
  DW1000.setDefaults();
  String msg = "{DD:0,AD:1,DW:0,AW:0,TD:0}";// msg += sentNum;
  DW1000.setData(msg);
  // delay sending the message for the given amount
  DW1000Time deltaTime = DW1000Time(10, DW1000Time::MILLISECONDS);
  DW1000.setDelay(deltaTime);
...
/*
Button_1 used for lock
Button_2 used for unlock
Button_3 used for trunk unlock
Button_4 used for hood unlock
Button_5 used for sunroof unlock
*/
//"{\"DD\":0,\"AD\":0,\"DW\":0,\"AW\":0,\"TD\":0,\"HD\":0,\"SD\":0,\"Dist\":00.00}"

int fob_transmit(int button)
{
  int temp_button = button;
  memset(tempBuff,'\0',sizeof(tempBuff));
  strcat(tempBuff, transmit_string);
  //distance function has to be brought in here
  if((temp_button&0x01) == 1)
  {
    memcpy(tempBuff+strlen("{\"DD\":0,\"AD\":"),"1",1);
    Serial.println(tempBuff);
    memset(in,0,sizeof(tempBuff));
    memcpy(in,tempBuff,sizeof(tempBuff));
    memcpy(out,tempBuff,sizeof(tempBuff));
  ...
  else if((temp_button&0x02) == 0x02)
  {
    memcpy(tempBuff+strlen("{\"DD\":0,\"AD\":"),"2",1);
    Serial.println(tempBuff);
    memset(in,0,sizeof(tempBuff));
    memcpy(in,tempBuff,sizeof(tempBuff));
    memcpy(out,tempBuff,sizeof(tempBuff));
  ...
  else if((temp_button&0x04) == 0x04)
  {
    memcpy(tempBuff+strlen("{\"DD\":0,\"AD\":0,\"DW\":0,\"AW\":0,\"TD\":"),"1",1);
    Serial.println(tempBuff);
    memset(in,0,sizeof(tempBuff));
    memcpy(in,tempBuff,sizeof(tempBuff));
    memcpy(out,tempBuff,sizeof(tempBuff));
  ...
  else if((temp_button&0x08) == 0x08)
  {
    memcpy(tempBuff+strlen("{\"DD\":0,\"AD\":0,\"DW\":0,\"AW\":0,\"TD\":0,\"HD\":"),"1",1);
    Serial.println(tempBuff);
    Serial.println(tempBuff);
    memset(in,0,sizeof(tempBuff));
    memcpy(in,tempBuff,sizeof(tempBuff));
  ...
  /*else if((temp_button&0x10) == 0x10)
  {
    Serial.println("Button_5 lock");
    transmitter();
  }*/
}
...
int keypad_scan()
{
    temp_read=0;
    BUTTON_state=0;
    temp_read=digitalRead(BUTTON_1);
    BUTTON_state=BUTTON_state|temp_read;
    
    temp_read=digitalRead(BUTTON_2);
    BUTTON_state=BUTTON_state|(temp_read<<1);

    temp_read=digitalRead(BUTTON_3);
    BUTTON_state=BUTTON_state|(temp_read<<2);

    temp_read=digitalRead(BUTTON_4);
    BUTTON_state=BUTTON_state|(temp_read<<3);

    //temp_read=digitalRead(BUTTON_5);
    //BUTTON_state=BUTTON_state|(temp_read<<4);
...
void loop() {
  ...
  Serial.println(key_result);
    if(key_result!=0)
    {
      sentAck = false;
      if(fob_transmit(key_result)==1)
      {
        transmitter();
      }
      else
      {
        //distance code to be brought in here also
      }
    }
    //transmitter();
  /*if(button_state_1==1)
  {
  sentAck = false;
  Serial.println("Driver Door key pressed");
 ...
 // sentTime = newSentTime;
  ...
  {
  sentAck = false;
  Serial.println("All Door key pressed");
  // update and print some information about the sent message
  //Serial.print("ARDUINO delay sent [ms] ... "); Serial.println(millis() - delaySent);
 // DW1000Time newSentTime;
  //DW1000.getTransmitTimestamp(newSentTime);
