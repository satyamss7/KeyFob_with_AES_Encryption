...
//#define LED_PIN    12  // ESP32 pin GIOP18, which connected to led
#include <stdio.h>
#include <string.h>
#include <stdint.h>
...
void loop() {
  ...
  if (received) {
    ...
    memset(in,0,message.length());
    memcpy(in,message.c_str(),message.length());
    Serial.println();
    //Serial.println((char*)in);
    test_decrypt_cbc();
    Serial.print("Decrypted data is...");Serial.println((char*)in);
